// criando variável para saber o tamanho da tela
...
// se página foi recarrega vai aparecer o calendário cadastrado
...
if (recarregou) {
  ...
  // remove a variável
  console.log("Antes de entrar no método de renderizar")
  document.addEventListener('load', renderCalendar(localStorage.getItem("meses"), localStorage.getItem("ano")))
  // executa sua função
} 

function controeCalendarios() {
  document.write(`<div class="meses">`);

  for (let i = 0; i < mes.length; i++) {
    document.write(
      `<div class="mes" id="mes_click" onclick="{pegaData(${mes[i][0]})}"><p class="nome-mes"><b>${mes[i][1]}</b></p></div>`
    );
  }
...
function constroeTopo() {
  document.write('<main id="content-calendarFull" >');
  document.write('<div class="topo-calendario">');
  document.write('<div class="topo">');
  document.write(
    '<img src="img/seta-esquerda.png" class="setas seta-esquerda" onclick="{diminueAno()}">'
  );
  document.write(`<h2 class="titulo" id="tituloYear">${ano}</h2>`);
  document.write(
    '<img src="img/seta-direita.png" class="setas seta-direita" onclick="{aumentaAno()}"></div>'
  );
  document.write("</div></div>");
}

for (let i = 0; i < mes.length; i++) {
  document.querySelectorAll("p")[i].value = mes[i][0];
}
...
// função que cria a modal
function criaModal() {
  document.write('<div id="modal-fecha" onclick="fechaModal()"></div>');
...
// método para abrir o calendário ao clicar no botão de submit
function renderCalendar(meses, ano) {
  abreModal();
  calendar(meses, ano);
  const calendario = document.getElementById('calendar')
  calendario.style.width = '50%'
  calendario.style.width = '60%'
}
